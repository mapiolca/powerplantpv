...
- Les dictionnaires maintenance `c_powerplantpv_intervention_nature`, `c_powerplantpv_maintenance_service`, `c_powerplantpv_report_section` et `c_powerplantpv_index_type` portent `entity` et sont déclarés comme dictionnaires Multicompany personnalisables par entité.
- Les liaisons maintenance stockent uniquement les clés `fk_maintenance_service` et `fk_report_section`; les libellés restent dans les dictionnaires sources.
...
| Socle données maintenance | v20.0 | Dictionnaires, droits et extrafields utilisent les APIs Dolibarr disponibles en v20. | Aucun écran complet de rapport, calcul de maintenance ou PDF de maintenance n'est fourni par cette étape. |
