...
| Signature en ligne d'attestation | v20.0 + `hash_file()` disponible + support core explicite de `powerplantpv_attestation` | La signature suit strictement le schéma natif Dolibarr `source/ref/securekey` via `/public/onlinesign/newonlinesign.php`. Le module ne fournit plus de page publique de remplacement. | Si `hash_file()` est indisponible ou si le core ne supporte pas la source `powerplantpv_attestation`, l'onglet Compatibilité signale l'indisponibilité de la signature en ligne et aucun lien n'est affiché. |
