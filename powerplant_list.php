...
/**
 *   	\file       powerplant_list.php
 *		\ingroup    powerplantpv
 *		\brief      List page for powerplant
 */
...
$permissiontodelete = powerplantpvUserHasRightPath($user, array('powerplantpv', 'powerplant', 'delete'));
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'PowerPlant';
	$objectlabel = 'PowerPlant';
	$uploaddir = $conf->powerplantpv->dir_output;
...
$title = $langs->trans("PowerPlants");
//$help_url = "EN:Module_PowerPlant|FR:Module_PowerPlant_FR|ES:Módulo_PowerPlant";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/powerplantpv/powerplant_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-powerplantpv page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/powerplantpv/powerplant_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendPowerPlantRef";
$modelmail = "powerplant";
$objecttmp = new PowerPlant($db);
