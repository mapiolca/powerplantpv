...
class pdf_attestation_bridage_dynamique extends pdf_attestation_base
{
	public function __construct($db)
	{
		...
		$this->setModelTranslationKeys('AttestationPdfModelBridageDynamiqueName', 'AttestationPdfModelBridageDynamiqueDescription');
	...
	protected function getFinalResultSignatureBlockHeight($pdf, $object, $outputlangs, $defaultFontSize, $derivedData, $attestationDate)
	{
		...
		$height += 6 + 5 + 4 + 1 + 20 + 2; // Signature block spacing, labels, signer line, boxes and trailing spacing.
	...
	protected function renderEquipmentTable($pdf, $object, $outputlangs, $defaultFontSize)
	{
		...
		$renderHeader = function () use ($pdf, $outputlangs, $widths, $headerValues, $fontSize, $headerStyles) {
			$this->renderEquipmentTableRow($pdf, $outputlangs, $widths, $headerValues, $fontSize, $headerStyles, true);
		};
		...
		foreach ($bodyRows as $row) {
			...
			$this->renderEquipmentTableRow($pdf, $outputlangs, $row['widths'], $row['values'], $fontSize, array(), false, false);
		}
		$pdf->Ln(2);
	}
	 ...
	 * @return	void
	 */
	protected function renderEquipmentTableRow($pdf, $outputlangs, $widths, $values, $fontSize, $styles = array(), $fill = false, $checkSpace = true)
	{
	...
	{
		$pdf->Ln(6);
		$this->renderNativeSignatureStampBoxes($pdf, $object, $outputlangs, $defaultFontSize, $derivedData, 'AttestationDynamicCompanySeal');
